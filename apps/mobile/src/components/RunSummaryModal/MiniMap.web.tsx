...
export function MiniMap({ coordinates }: Props) {
  ...
  if (pts.length < 2) {
    return (
      <View
        style={{ width: '100%', height: H, borderRadius: 16, backgroundColor: '#27272a' }}
        className="items-center justify-center"
      >
        <Text className="text-zinc-500 text-sm">경로 데이터 없음</Text>
      </View>
    );
  }
  ...
  return (
    <View
      style={{ width: '100%', height: H, borderRadius: 16, backgroundColor: '#18181b', overflow: 'hidden' }}
    >
      {/* 선분을 절대위치 View로 표현 */}
      ...
      <View
        style={{
          position: 'absolute',
          left: pts[pts.length - 1].x - 5,
          top: pts[pts.length - 1].y - 5,
          width: 10, height: 10,
          borderRadius: 5,
