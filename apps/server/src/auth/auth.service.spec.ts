...
const mockRefreshTokenModel = { findOne: jest.fn(), create: jest.fn() };
const mockUsersService = { findByEmail: jest.fn(), findById: jest.fn(), create: jest.fn() };
const mockJwtService = { sign: jest.fn().mockReturnValue('mocked-token'), verify: jest.fn() };
...
describe('AuthService', () => {
  ...
  describe('register', () => {
    it('새 유저를 생성하고 토큰+유저 반환', async () => {
      mockUsersService.findByEmail.mockResolvedValue(null);
      mockUsersService.create.mockResolvedValue({ _id: 'u1', email: 'a@b.com', username: 'alice' });
      ...
      expect(mockUsersService.create).toHaveBeenCalledWith('a@b.com', expect.any(String), 'alice');
    });

    it('이미 존재하는 이메일이면 ConflictException', async () => {
      mockUsersService.findByEmail.mockResolvedValue({ _id: 'u1' });
      await expect(service.register('alice', 'a@b.com', 'pw')).rejects.toThrow(ConflictException);
    });
    ...
    it('비밀번호를 해싱해서 저장', async () => {
      ...
      await service.register('alice', 'a@b.com', 'plaintext');

      const hashedPw = mockUsersService.create.mock.calls[0][1];
      expect(await bcrypt.compare('plaintext', hashedPw)).toBe(true);
    });
  });

  describe('login', () => {
    it('유효한 인증 정보로 토큰+유저 발급', async () => {
      const user = { _id: { toString: () => 'u1' }, email: 'a@b.com', username: 'alice', passwordHash: await bcrypt.hash('pw', 10) };
    ...
    it('비밀번호 틀리면 UnauthorizedException', async () => {
      const user = { _id: { toString: () => 'u1' }, email: 'a@b.com', passwordHash: await bcrypt.hash('correct', 10) };
      mockUsersService.findByEmail.mockResolvedValue(user);
  ...
  describe('refresh', () => {
    it('유효한 리프레시 토큰으로 새 토큰 발급', async () => {
      ...
      const storedToken = { expiresAt: new Date(Date.now() + 60000), deleteOne: jest.fn().mockResolvedValue({}) };
    ...
    it('만료된 토큰이면 UnauthorizedException', async () => {
      mockJwtService.verify.mockReturnValue({ sub: 'u1' });
      mockRefreshTokenModel.findOne.mockResolvedValue({ expiresAt: new Date(Date.now() - 1000), deleteOne: jest.fn() });
